...
export default function App() {
  ...
  const handleSubmit = async() => {
    ...
    formData.append('object', object);
    if(action === 'update'){
      let res = await axios.put(`${process.env.API_URL}/update-data` , formData );
      console.log(res);
    }
    if(action === 'add'){
      let res = await axios.post(`${process.env.API_URL}/add-data` ,formData );
      console.log(res);
    }
    if(action === 'delete'){
      let res = await axios.delete(`${process.env.API_URL}/delete-data` , formData);
      console.log(res);
    }
    if(action === 'prop'){
      let res = await axios.put(`${process.env.API_URL}/update-properties` , formData);
      console.log(res);
    }
    if(action === 'map'){
      let res = await axios.put(`${process.env.API_URL}/map-info` , formData);
      console.log(res);
    }
  }
